    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-end border-b border-white/10 pb-5">
        ...
        <button
          ...
          className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 text-white font-semibold text-sm shadow-lg shadow-indigo-500/30 hover:opacity-90 transition-all flex items-center gap-2"
        >
          {sendingEmail ? (
            <>
              <Sparkles className="w-4 h-4 animate-spin" />
              <span>Sending Report via Resend...</span>
            </>
          ) : (
            <>
      ...
      {emailSentSuccess && (
        <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-sm flex items-center gap-3">
          ...
          <span>Executive report email has been rendered and queued for immediate delivery via Resend.</span>
        </div>
      )}
          ...
          <div>
            <div className="flex justify-between items-center pb-4 border-b border-white/10 mb-6">
              ...
              <span className="text-xs text-gray-400 flex items-center gap-1.5 font-mono">
                <Clock className="w-3.5 h-3.5 text-cyan-400" /> Scheduled: Daily 06:00 AM UTC
              </span>
            </div>
